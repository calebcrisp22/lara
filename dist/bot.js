import { Client, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, EmbedBuilder, GatewayIntentBits, PermissionFlagsBits, REST, Routes, SlashCommandBuilder, } from "discord.js";
...
const commandDefinitions = commands.map(([name, description, option, permission]) => ({
    name,
    description,
    option,
    permission,
...
function commandBuilder(command) {
    ...
    if (command.permission)
        builder.setDefaultMemberPermissions(command.permission);
    if (command.option === "user") {
        builder.addUserOption((option) => option.setName("user").setDescription("Choose a member").setRequired(true));
    }
    else if (command.option === "number") {
        builder.addIntegerOption((option) => option.setName("amount").setDescription("Enter a number").setRequired(true).setMinValue(1));
    }
    else if (command.option === "string") {
        builder.addStringOption((option) => option.setName("value").setDescription("Enter a value").setRequired(true));
    }
    return builder;
}
...
async function handleCommand(interaction) {
    ...
    if (command === "ticketpanel") {
        ...
        const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("ticket_owner").setLabel("👑 Speak to Owner").setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId("ticket_support").setLabel("🛠️ Support").setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId("ticket_purchase").setLabel("💰 Purchase").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId("ticket_website").setLabel("🌐 Website Purchased").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("ticket_problem").setLabel("⚠️ Problem with Purchase").setStyle(ButtonStyle.Danger));
